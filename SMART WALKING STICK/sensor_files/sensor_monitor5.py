...
class SensorMonitor:
    def __init__(self):
        try:
            ...
            gps_success = False
            retry_count = 0
            while not gps_success and retry_count < 20:
                if self.gps.init_module():
                    ...
                    retry_count += 1
                    print(f"Retry {retry_count}/20: Failed to initialize GPS/GSM module")
                    time.sleep(2)
    ...
    def monitor_button(self):
        """Monitor button presses and handle voice commands."""
        button_hold_time = 0
        listening = False
        ...
        while self.running:
            try:
                ...
                if not GPIO.input(BUTTON_PIN):
                    ...
                    if not listening and current_time - self.last_button_press >= 0.5:
                        try:
                            ...
                            if self.speech.start_listening():
                                self.speech.speak("I'm listening", 'system', min_interval=1)
                            else:
                    ...
                    # Continue listening while button is held
                    if listening:
                        try:
                                ...
                                if command:
                                    ...
                                    if not self.speech.process_voice_command(command, self):
                                        if "help" in command.lower():
                                            ...
                                            # Use existing command processing
                                            prompt = self.speech.get_random_prompt()
                                            if prompt:
                                                self.speech.speak(prompt, 'general')
                ...
                # Button released
                    ...
                    if listening:
                        try:
                            ...
                            hold_duration = time.time() - button_hold_time
                            
                            # Only acknowledge if held for more than 0.5 seconds
                            if hold_duration >= 0.5:
                                self.speech.speak("Ready for your next command", 'system', min_interval=2)
                        except Exception as e:
                            print(f"Error stopping voice recognition: {e}")
                    
                    time.sleep(0.05)  # Short sleep while waiting for button press
            ...
            try:
                ...
                if current_time - self.last_network_check >= 60:  # Check every minute
                    ...
                    if not network_status:
                        self.speech.speak("Warning: No cellular network connection. Emergency alerts may not work properly.", 'network', min_interval=300)
                        self.firebase.send_system_alert("Network Warning", "No cellular network connection")
                    elif signal_strength < 10:
                        self.speech.speak("Warning: Poor network signal strength. Emergency alerts may be delayed.", 'network', min_interval=300)
                        self.firebase.send_system_alert("Network Warning", f"Poor signal strength: {signal_strength}")
